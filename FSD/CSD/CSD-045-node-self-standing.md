...
| where `delegation_id` comes from | `owner_delegations` on `GET /v1/admin/self`: the owner-issued `delegates_to` rows carrying `infra:serve` whose subject is this node, filtered by `resolve_owner_authority`, folded over all of the node's own keys on a split install (triage comment on CIRISServer#676, 2026-09-25) | CIRISServer | **in build → built, 0.5.218, unmerged** (CIRISServer#676, still open; the brief's update: "`GET /v1/admin/self` returns `owner_delegations`. Also fixes admin acts on split installs"). Not readable: no branch or PR is pushed, and `main` @ `046e1b39` has no `owner_delegations`. The item shape (a bare id, or an id with scope and expiry) is **unconfirmed** until CIRISClient#78. With it, the card sends the one usable id itself and the person never pastes one. On `main` the client still has no read for it (`GET /v1/auth/device/grants` lists grants, not the owner's `delegates_to` to the node), so this row still blocks `sketched` until the release ships |
