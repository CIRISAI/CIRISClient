...
| attach this device to a Portal-registered node | `POST /v1/setup/connect-node` — SSRF-validated, persists the session, returns the verification URL | CIRISServer `src/auth/device_auth.rs:371` | live, **wired and unreached.** `CIRISApiClient.connectToNode` (`CIRISApiClient.kt:7325`) exists; its only consumer is a function parameter of `SetupViewModel.startNodeConnection` (`SetupViewModel.kt:2246`), and nothing calls `startNodeConnection`. The route-coverage report marked this CALLED |
| poll that attachment | `GET /v1/setup/connect-node/status` — 428 pending, 403 denied, 200 complete | CIRISServer `src/auth/device_auth.rs:373` | live, **wired and unreached** — `CIRISApiClient.pollNodeAuthStatus` (`CIRISApiClient.kt:7374`) has no caller, and `SetupViewModel.pollNodeAuthStatus` (`SetupViewModel.kt:2288`), which takes such a function, is never invoked |
| forget a half-finished attachment | `POST /v1/setup/reset-device-auth` — no auth | CIRISServer `src/auth/device_auth.rs:377` | live, **wired and unreached** — `resetDeviceAuthOnServer` (`CIRISApiClient.kt:7449`) has no caller. Also marked CALLED in the report |
