...
- Say what this pipeline does not guarantee, every time: native fluency, dialect coverage, cultural adaptation of metaphor, legal review. Everything it writes is `draft` / `machine_reviewed` — machine-translated and MQM-reviewed by a judge of a different model family than the writer. There is NO native-reviewer pipeline, so do not write `needs_native_review` or otherwise imply a human sign-off is queued; state the limit instead of promising a review that is not coming.
