...
.rings{display:grid;gap:9px;margin:14px 0 4px}
.ring{display:flex;gap:12px;align-items:flex-start;background:var(--bg);
  border:1px solid var(--line);border-radius:10px;padding:11px 13px}
.ring .n{flex:0 0 26px;height:26px;border-radius:50%;background:var(--accent);color:#fff;
  display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700}
.ring b{font-size:14px}
.ring code{display:block;font-size:11px;color:var(--muted);margin:1px 0 4px}
.ring p{margin:0;font-size:13px;color:var(--muted);max-width:66ch}
...
<div class="wrap">
  ...
  <main>
    <div class="tabs">
      <div class="tab on" data-view="start">Start here</div>
      <div class="tab" data-view="screens">Screens</div>
...
function renderStart(){
  const m = document.getElementById('main'); const e = DATA.explainer;
  if (!e){ m.innerHTML = '<p class="note">No explainer in this manifest.</p>'; return; }
  m.innerHTML = `<h2 style="font-size:19px;margin:0 0 4px;letter-spacing:-.01em">
      What this is for</h2>
    <p class="note" style="margin:0 0 18px;max-width:64ch">Read this before the
    screenshots. It is the shortest path from what CIRIS believes to what the
    interface has to do about it.</p>`;
  const wrap = document.createElement('div'); wrap.className = 'why';
  for (const sec of e.sections){
    const el = document.createElement('div'); el.className = 'doc';
    let html = `<h3 style="font-size:16px;margin-bottom:8px">${sec.h}</h3>`;
    for (const para of sec.body) html += `<p style="max-width:72ch">${para}</p>`;
    if (sec.circles){
      html += '<div class="rings">';
      e.circles.forEach((c, i) => {
        html += `<div class="ring"><span class="n">${i+1}</span>
          <div><b>${c[0]}</b><code>${c[1]}</code><p>${c[2]}</p></div></div>`;
      });
      html += '</div>';
    }
    el.innerHTML = html; wrap.appendChild(el);
  }
  m.appendChild(wrap);

  const h = document.createElement('h2');
  h.style.cssText = 'font-size:15px;margin:26px 0 4px';
  h.textContent = 'Questions a designer asked, answered by name';
  m.appendChild(h);
  const p2 = document.createElement('p'); p2.className = 'note';
  p2.style.cssText = 'margin:0 0 12px;max-width:72ch';
  p2.innerHTML = 'This page was tested by having it rewritten for a five-year-old, '
    + 'then reconstructed by readers who saw only that. Nearly everything survived — '
    + 'the ideas are not hard. These are the places where the reconstruction hit a wall '
    + 'because a real mechanism had never been named.';
  m.appendChild(p2);
  const w2 = document.createElement('div'); w2.className = 'why';
  for (const pair of e.answered){
    const el = document.createElement('div'); el.className = 'doc';
    el.innerHTML = `<h3>${pair[0]}</h3><p style="max-width:72ch">${pair[1]}</p>`;
    w2.appendChild(el);
  }
  m.appendChild(w2);
}
...
function renderWhy(){
  ...
  m.innerHTML = `<h2 style="font-size:15px;margin:0 0 6px">Why it is shaped this way</h2>
    <p class="note" style="margin:0 0 4px">The shape is not an engineering convention.
    It follows from three commitments — everyone is equal, privacy is appropriate flow
    rather than secrecy, and what you can see depends on the attestations in play.
    Each one rules out a familiar UI move.</p>`;

  const wrap = document.createElement('div'); wrap.className='why'; wrap.style.marginTop='16px';
  for (const d of (DATA.principles || [])){
    const el = document.createElement('div'); el.className='doc';
    el.innerHTML = `<h3>${d.title}</h3>
      <div class="where"><a href="${d.url}" target="_blank" rel="noopener">${d.source}</a></div>
      <blockquote style="margin:8px 0;padding-left:12px;border-left:3px solid var(--accent);
        font-size:13.5px;line-height:1.55">${d.quote}</blockquote>
      <p>${d.gist}</p>
      <p class="mu"><b>What this forces in the UI:</b> ${d.ui}</p>`;
    wrap.appendChild(el);
  }
  m.appendChild(wrap);

  if (DATA.design_rules && DATA.design_rules.length){
    const h = document.createElement('h2');
    h.style.cssText='font-size:15px;margin:24px 0 6px';
    h.textContent = 'Rules a redesign must not break';
    m.appendChild(h);
    const w = document.createElement('div'); w.className='why';
    for (const r of DATA.design_rules){
      const el = document.createElement('div'); el.className='doc';
      el.innerHTML = `<h3>${r[0]}</h3><p>${r[1]}</p>`;
      w.appendChild(el);
    }
    m.appendChild(w);
  }
  ...
  if (ns.families){
    const h = document.createElement('h2');
    h.style.cssText='font-size:15px;margin:24px 0 6px';
    h.textContent = 'The vocabulary the UI names its fields from';
    m.appendChild(h);
    ...
    note.innerHTML = `A field is named by its constitutional family, because the family is the `
      + `<b>information type</b> — one of the five parameters that decide whether a flow is `
      + `appropriate. Counts are read from <code>manifests/namespace_registry.json</code>, `
      + `generated from <code>${ns.source || ''}</code>; the Constitution says to take them from `
      + `there and never from a number restated in prose.`;
    m.appendChild(note);
  }
  ...
  if (DATA.lineage && DATA.lineage.length){
    const h = document.createElement('h2');
    h.style.cssText='font-size:15px;margin:24px 0 6px';
    h.textContent = 'Where it is written down';
    m.appendChild(h);
    const p2 = document.createElement('p'); p2.className='note';
    p2.style.margin='0 0 12px';
    p2.textContent = 'The specifications that carry the commitments above into the build. '
      + 'Useful when you need the exact wording; not the reason itself.';
    m.appendChild(p2);
    const w = document.createElement('div'); w.className='why';
  ...
  if (DATA.observations && DATA.observations.length){
    const h = document.createElement('h2');
    h.style.cssText = 'font-size:15px;margin:24px 0 6px';
    h.textContent = 'What the atlas found in the nav itself';
    m.appendChild(h);
    const w2 = document.createElement('div'); w2.className = 'why';
...
def main() -> int:
    ...
    from testing.gate import atlas_explainer
    manifest["explainer"] = {
        "circles": atlas_explainer.CIRCLES,
        "sections": atlas_explainer.SECTIONS,
        "answered": atlas_explainer.ANSWERED,
    }
    manifest["principles"] = atlas_context.PRINCIPLES
    manifest["design_rules"] = atlas_context.DESIGN_RULES
